...
// ACT-detector: changes in the following functions 
...
template void ACTComputeConfLossGPU(const Blob<float>& conf_data, const int num,
      const int num_preds_per_class, const int num_classes,
      const int background_label_id, const ConfLossType loss_type,
      const vector<map<int, vector<int> > >& all_match_indices,
      const map<int, vector<ACTNormalizedTube> >& all_gt_tubes,
      vector<vector<float> >* all_conf_loss);
template void ACTComputeConfLossGPU(const Blob<double>& conf_data, const int num,
      const int num_preds_per_class, const int num_classes,
      const int background_label_id, const ConfLossType loss_type,
      const vector<map<int, vector<int> > >& all_match_indices,
      const map<int, vector<ACTNormalizedTube> >& all_gt_tubes,
...
__global__ void ACTDecodeTubesKernel(const int nthreads,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    // ACT-detector: i: [0,..,3], ie, the [x1,y1,x2,y2] coordinates of a box in the tube
    const int i = index % 4 ; 
    // ACT-detector: id of the class if per-class regression 
    const int c = (index / (4*sequence_length) ) % num_loc_classes; 
    // ACT-detector: index of the prior box
    const int d = (index / (4*sequence_length) / num_loc_classes) % num_priors; 
    if (!share_location && c == background_label_id) {
      // Ignore background class if not share_location.
      return;
    }
    const int pi = d * 4; // ACT-detector: there is one prior box per tube
    const int vi = pi + num_priors * 4; 
    if (code_type == PriorBoxParameter_CodeType_CORNER) {
      // ACT-detector: in the main function: CHECK(false) << "ACT-detector: Not Implemented";       
    } else if (code_type == PriorBoxParameter_CodeType_CENTER_SIZE) {
      const Dtype p_xmin = prior_data[pi];
      const Dtype p_ymin = prior_data[pi + 1];
      const Dtype p_xmax = prior_data[pi + 2];
...
void ACTDecodeTubesGPU(const int nthreads,
  ...
  // NOLINT_NEXT_LINE(whitespace/operators)
  ACTDecodeTubesKernel<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, loc_data, prior_data, code_type,
      variance_encoded_in_target, num_priors, share_location, num_loc_classes,
      background_label_id, clip_bbox, bbox_data, 
      sequence_length); 
...
template void ACTDecodeTubesGPU(const int nthreads,
          const float* loc_data, const float* prior_data,
          const CodeType code_type, const bool variance_encoded_in_target,
          const int num_priors, const bool share_location,
          const int num_loc_classes, const int background_label_id,
